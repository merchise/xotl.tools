...
There is an amalgam of methods to validate functions results in a "safe" way
when there are possibilities of failures.  The best, or more clear method, is
to raise an exception, but there are scenarios where a fail could be signaled
with "special values".
