...
- [ ] Manage deprecation warnings (code and documentation) for: [0/2]

  - [ ] 'xoutil.future.string' must be fully deprecated.

    See [[file:xoutil/future/string.py][module]].  There are some useful tools in there that must be migrated to
    [[file:xoutil/future/textwrap.py][xoutil.future.textwrap]] and a new one [[file:xoutil/future/codecs.py][xoutil.future.codecs]].
